<script>
    ...
    function update() {
        callback && callback({
            ...
            secret: {
                ...
                defaultMode: props.secret.defaultMode,
                optional: props.secret.optional,
                items: items.map(a => {
                    return {
                        key: a.key,
                        path: a.path,
        ...
        if (value) {
            props = {
                ...
                secret: {
                    ...
                    defaultMode: value.secret.defaultMode,
                    optional: value.secret.optional,
                    items: value.secret.items || []
                }
            };
        }
    })
    $effect(() => {
        items = props.secret.items.map(a => {
    ...
    $effect(() => {
        if (props.secret.secretName) {
            let t = secretList.find(a => a.name === props.secret.secretName);
            if (t) {
                secretKeys = Reflect.ownKeys(t.data).map(a => {
...
<div class="section">
    ...
    <div class="block colum-2">
        ...
        <div class="field">
            <Select label="Secret Name" options={secretList} bind:value={props.secret.secretName} onClose={update}
                    disabled={disabled}/>
        </div>
    </div>
    <div class="block colum-2">
        <div class="field">
            <Input label="defaultMode" bind:value={props.secret.defaultMode} onblur={update} disabled={disabled}/>
        </div>
        <div class="field">
            <div class="to-left">
                <RadioGroup options={options} bind:value={props.secret.optional} callback={update} disabled={disabled}/>
            </div>
        </div>
    </div>
    {#each items as item}
        <div class="block env-grid">
            <div class="field">
                <Select label="Key" options={secretKeys} bind:value={item.key} onClose={update} disabled={disabled}/>
            </div>
            <div class="field">
