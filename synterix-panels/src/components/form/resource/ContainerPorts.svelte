<script>
    ...
    function update() {
        let t = portList.map(({name, containerPort, protocol}) => {
            return {name, containerPort, protocol};
        });
        callback && callback(t);
    }
...
<div class="section">
    ...
    {#each portList as port}
        <div class="block port-grid">
            ...
            <div class="field">
                <Select label="Protocol" bind:value={port.protocol} options={protocols} disabled={disabled}/>
            </div>
            <div class="field">
                <Input label="Container Port" bind:value={port.containerPort} onblur={()=>update()}
