        ...
        if (value) {
            mapList = value.map(a => {
                return {
                    weight: a.weight,
                    preference: {
                        matchExpressions: a.preference.matchExpressions || []
                    },
                    id: Math.random()
                }
            });
    ...
    {#each mapList as map}
        <div class="panel">
            <div class="content">
                ...
                <div class="block  colum-1">
                    <Rule label="MatchExpressions" bind:value={map.preference.matchExpressions}
                          callback={v=>(map.preference.matchExpressions=v)&&update()} disabled={disabled}/>
                </div>
            </div>
        </div>
    {/each}
